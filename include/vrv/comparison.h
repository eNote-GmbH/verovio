...
class VerseLikeComparison : public Comparison {
public:
    explicit VerseLikeComparison(int groupN) : m_groupN(groupN) {}

    bool operator()(const Object *object) override;
    bool MatchesType(const Object *object) const { return object->IsAnyOf(std::array{ REFRAIN, VERSE }); }
...
public:
    ...
    bool Apply(const Object *object) const
    {
        auto condition = [object](Comparison *iter) {
            ...
            VerseLikeComparison *verseLikeCmp = dynamic_cast<VerseLikeComparison *>(iter);
            if (verseLikeCmp) return verseLikeCmp->MatchesType(object) ? (*iter)(object) : true;
            return (*iter)(object);
        };
        switch (m_type) {
            case Type::AnyOf: {
