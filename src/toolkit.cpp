...
{
    Resources &resources = m_doc.GetResourcesForModification();
    const std::string runtimeFontName = (fontName == "Times") ? "Tinos" : fontName;
    const bool ok = resources.GetFontStore().HasFace(FontStore::Kind::Text, runtimeFontName);
    if (ok) {
        this->InvalidateSvgCache();
        resources.SetTextFont(runtimeFontName);
        if (this->GetPageCount() > 0) m_fontLayoutInvalid = true;
    }
    else {
        LogWarning("Text font '%s' has not been registered", fontName.c_str());
    }
    return ok;
}
...
void Toolkit::ResetOptions()
{
    ...
    // Set the default runtime music and text fonts
    this->SetFont(m_options->m_font.GetValue());
    this->SetTextFont(m_options->m_fontText.GetValue());
}
...
{
    if (m_doc.AbortRequested()) return "";
