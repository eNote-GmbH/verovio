        ...
        for (layers = staves->second.child.begin(); layers != staves->second.child.end(); ++layers) {
            for (verses = layers->second.child.begin(); verses != layers->second.child.end(); ++verses) {
                for (voltaTracks = verses->second.child.begin(); voltaTracks != verses->second.child.end();
                    ++voltaTracks) {
                    // Create comparisons for staff/layer and the internal verse-like group.
                    AttNIntegerComparison matchStaff(STAFF, staves->first);
                    AttNIntegerComparison matchLayer(LAYER, layers->first);
                    VerseLikeComparison matchVerseLike(verses->first);
                    filters = { &matchStaff, &matchLayer, &matchVerseLike };
