...
{
    VerseLike *verseLike = dynamic_cast<VerseLike *>(container);
    if (verseLike) return verseLike->AdjustPosition(overlap, freeSpace, doc);
...
static int GetVoltaGroupingSymbolWidth(const VerseLike *verseLike, int unit)
{
    if ((verseLike->GetVoltaCount() < 2) || !verseLike->HasVoltasym()) return 0;
...
{
    return this->VisitVerseLike(verse);
}

FunctorCode AdjustSylSpacingFunctor::VisitRefrain(Refrain *refrain)
{
    return this->VisitVerseLike(refrain);
}

FunctorCode AdjustSylSpacingFunctor::VisitVerseLike(VerseLike *verseLike)
{
    ...
    LayerElement *container = verseLike;
    ListOfObjects syls;
    if (m_voltaTrack == 0) {
        syls = verseLike->FindAllDescendantsByType(SYL);
        ...
        if (container == verseLike) return FUNCTOR_CONTINUE;
    }
    ...
    const int groupingSymbolWidth = (m_voltaTrack > 0) ? GetVoltaGroupingSymbolWidth(verseLike, shift) : 0;
