...
bool Doc::GenerateDocumentScoreDef()
{
    ...
    GetCurrentScoreDef()->AddChild(staffGrp);
...
{
    if (GetCurrentScoreDef()->FindDescendantByType(PGFOOT)) {
        return false;
    }
...
{
    if (GetCurrentScoreDef()->FindDescendantByType(PGHEAD)) return false;
    ...
    GetCurrentScoreDef()->AddChild(pgHead);
    ...
    GetCurrentScoreDef()->AddChild(pgHead2);
...
void Doc::CalculateMidiTimemap()
{
    ...
    // Set tempo
    if (GetCurrentScoreDef()->HasMidiBpm()) {
        tempo = GetCurrentScoreDef()->GetMidiBpm();
    }
...
void Doc::ExportMIDI(smf::MidiFile *midiFile)
{
    ...
    for (staves = prepareProcessingListsParams.m_layerTree.child.begin();
        ...
        int transSemi = 0;
        if (StaffDef *staffDef = GetCurrentScoreDef()->GetStaffDef(staves->first)) {
            ...
            MeterSig *meterSig = dynamic_cast<MeterSig *>(GetCurrentScoreDef()->FindDescendantByType(METERSIG));
...
void Doc::TransposeDoc()
{
    ...
    if (transposer.IsValidIntervalName(transpositionOption)) {
        ...
        KeySig *keysig = dynamic_cast<KeySig *>(GetCurrentScoreDef()->FindDescendantByType(KEYSIG));
        ...
        KeySig *keysig = dynamic_cast<KeySig *>(GetCurrentScoreDef()->FindDescendantByType(KEYSIG, 3));
        int fifths = 0;
        if (keysig) {
    ...
    GetCurrentScoreDef()->Process(&transpose, &transposeParams);
...
double Doc::GetStaffDistance(const ClassId classId, int staffIndex, data_STAFFREL staffPosition)
{
    ...
    if (staffPosition == STAFFREL_above || staffPosition == STAFFREL_below) {
        if (classId == DYNAM) {
            ...
            if (GetCurrentScoreDef()->HasDynamDist()) {
                distance = GetCurrentScoreDef()->GetDynamDist();
            }

            // Inspect the staffDef attributes
            const StaffDef *staffDef = GetCurrentScoreDef()->GetStaffDef(staffIndex);
            ...
            // Inspect the scoreDef attribute
            if (GetCurrentScoreDef()->HasHarmDist()) {
                distance = GetCurrentScoreDef()->GetHarmDist();
            }
