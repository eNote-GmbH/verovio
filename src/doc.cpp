...
void Doc::CastOffDocBase(bool useSb, bool usePb)
{
    ...
    for (int i = 0; i < systemCount; i++) {
        ...
        if (useSb && !usePb) {
            CastOffEncodingParams castOffEncodingParams(this, contentPage, currentSystem, contentSystems[i], false);

            Functor castOffEncoding(&Object::CastOffEncoding);
            contentSystems[i]->Process(&castOffEncoding, &castOffEncodingParams);
        }
        else {
