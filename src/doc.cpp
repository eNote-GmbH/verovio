...
{
    bool res = true;
    std::vector<Pages *> pagesList = GetPagesList();
    for (Pages *pages : pagesList) {
        ...
        ScoreDef *scoreDef = mdiv->m_referenceScoreDef;
        assert(scoreDef);
        if (scoreDef->FindDescendantByType(PGFOOT) || m_options->m_adjustPageHeight.GetValue()) {
            res &= false;
        }
...
bool Doc::GenerateHeader()
{
    ...
    for (Pages *pages : pagesList) {
        assert(pages);
        Mdiv *mdiv = pages->GetParentMdiv();
        assert(mdiv && mdiv->m_visibility == Visible);
        ScoreDef *scoreDef = mdiv->m_referenceScoreDef;
        assert(scoreDef);
        if (scoreDef->FindDescendantByType(PGHEAD)) return false;
...
void Doc::CalculateMidiTimemap()
{
    ...
    if (m_pages && !m_drawingPage && GetPageCount() == 1) {
        Page *page = this->SetDrawingPage(m_pages->GetParentMdiv(), 0);
        if (!page) {
            return;
        }
...
void Doc::ExportMIDI(smf::MidiFile *midiFile)
{
    ...
    for (staves = prepareProcessingListsParams.m_layerTree.child.begin();
        ...
        if (StaffDef *staffDef = this->m_scoreDef.GetStaffDef(staves->first)) {
            ...
            MeterSig *meterSig = dynamic_cast<MeterSig *>(this->m_scoreDef.FindDescendantByType(METERSIG));
...
void Doc::CastOffDoc()
{
    ...
    for (Pages *pages : pagesList) {
        ...
        Page *currentPage = new Page(mdiv);
        CastOffPagesParams castOffPagesParams(pages, contentPage, this, currentPage);
...
{
    Pages *pages = params->m_currentPages;
    assert(pages);

    Mdiv *mdiv = pages->GetParentMdiv();
    assert(mdiv);

    Page *page1 = new Page(mdiv);
    pages->AddChild(page1);
    this->SetDrawingPage(mdiv, 0);
    ...
    Page *page2 = new Page(mdiv);
    pages->AddChild(page2);
    this->SetDrawingPage(mdiv, 1);
...
void Doc::UnCastOffDoc()
{
    ...
    for (Pages *pages : pagesList) {
        ...
        Page *contentPage = new Page(pages->GetParentMdiv());
...
void Doc::CastOffEncodingDoc()
{
    ...
    for (Pages *pages : pagesList) {
        ...
        Page *contentPage = this->SetDrawingPage(pages->GetParentMdiv(), 0);
        assert(contentPage);
        ...
        Page *page = new Page(pages->GetParentMdiv());
        pages->AddChild(page);
...
void Doc::ConvertToPageBasedDoc()
{
    ...
    for (int i = 0; i < scores.size(); i++) {
        ...
        Pages *pages = new Pages(mdiv);
        pages->ConvertFrom(score);
        Page *page = new Page(mdiv);
        pages->AddChild(page);
        System *system = new System();
        page->AddChild(system);
...
bool Doc::HasPage(Mdiv *mdiv, int pageIdx)
{
    Pages * pages = this->GetPages(mdiv);
    assert(pages);
...
{
    ClassIdComparison matchType(PAGES);
    Object * pagesObject = mdiv->FindDescendantByComparison(&matchType);
    Pages *pages = dynamic_cast<Pages *>(pagesObject);
    assert(pages);
    return pages;
}

int Doc::GetPageCount(int mdivNumber)
{
    std::vector<Pages *> pages = this->GetPagesList();
    assert(static_cast<int>(pages.size()) > mdivNumber);
    int rc;
    if (mdivNumber < 0) {
        rc = 0;
        std::for_each(pages.begin(), pages.end(), [&rc](Pages *p) { rc += p->GetParentMdiv()->m_visibility == Visible ? p->GetChildCount() : 0; });
    } else {
        rc = pages[mdivNumber]->GetChildCount();
    }
    return rc;
}
...
Page *Doc::SetDrawingPage(Mdiv *mdiv, int pageIdx)
{
    // out of range
    if (!HasPage(mdiv, pageIdx)) {
        return NULL;
    }
    Pages * pages = this->GetPages(mdiv);
    assert(pages);
    // nothing to do
    if (m_pages && m_drawingPage && m_pages == pages && m_drawingPage->GetIdx() == pageIdx) {
